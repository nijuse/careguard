...
function rebuildScrubbedSystemPrompt() {
  _scrubSession = _piiScrub
    ? buildScrubSession([profileData.recipient.name], [profileData.caregiver.name])
    : null;
  ...
  try {
    ...
    switch (name) {
      case 'compare_pharmacy_prices':
        result = await comparePharmacyPrices(input.drug_name as string, input.zip_code as string);
        break;
      ...
      case 'pay_for_medication':
        result = await payForMedication(
          input.pharmacy_id as string,
          input.pharmacy_name as string,
          input.drug_name as string,
          parseFloat(input.amount as string),
        );
        break;
      case 'pay_bill':
        result = await payBill(
          input.provider_id as string,
          input.provider_name as string,
          input.description as string,
          parseFloat(input.amount as string),
        );
        break;
      case 'check_spending_policy':
        result = checkSpendingPolicy(parseFloat(input.amount as string), input.category as 'medications' | 'bills');
        break;
      ...
      case 'check_adherence':
        result = checkAdherence(String(input.recipient_id || ''));
        break;
      default:
